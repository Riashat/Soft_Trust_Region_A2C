...
def main():
    ...
    for j in range(num_updates):
        ...
        values, action_log_probs, dist_entropy, states, all_log_probs = actor_critic.evaluate_actions(Variable(rollouts.observations[:-1].view(-1, *obs_shape)),
        ...
        """
        target_values, target_action_log_probs, target_dist_entropy, target_states, target_all_log_probs = target_actor_critic.evaluate_actions(Variable(rollouts.observations[:-1].view(-1, *obs_shape)),
                                                                                       Variable(rollouts.states[0].view(-1, actor_critic.state_size)),
                                                                                       Variable(rollouts.masks[:-1].view(-1, 1)),
                                                                                       Variable(rollouts.actions.view(-1, action_shape)))

        actor_regularizer_loss = (torch.exp(all_log_probs) * (all_log_probs - target_all_log_probs)).sum(1).mean()
        ...
        action_loss = -(Variable(advantages.data) * action_log_probs).mean() + args.actor_lambda * actor_regularizer_loss
        ...
        """
        NEED TO DO SOFT UPDATE OF THE PARAMETERS HERE?
        """
